...
const SingleVLQReconstructionHypothesis * get_best_hypothesis(const std::vector<SingleVLQReconstructionHypothesis> & hyps, const std::string & label){
  const SingleVLQReconstructionHypothesis * best = nullptr;
...
SingleVLQChi2Discriminator::SingleVLQChi2Discriminator(Context & ctx, const cfg & config_): config(config_){
  h_hyps = ctx.get_handle<vector<SingleVLQReconstructionHypothesis>>("TprimeHypotheses");
}


bool SingleVLQChi2Discriminator::process(uhh2::Event & event){
