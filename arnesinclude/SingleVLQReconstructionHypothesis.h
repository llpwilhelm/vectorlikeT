 ...
 * SingleVLQReconstructionHypothesis then consists of finding the lepton, and assigning the jets of the event
 ...
 * see SingleVLQReconstructionHypothesisDiscriminators.h for different criteria and to fill the discriminators.
 */
class SingleVLQReconstructionHypothesis {
public:
  ...
  float discriminator(const std::string & l) const {
      ...
      if(it == m_discriminators.end()){
          throw std::runtime_error("SingleVLQReconstructionHypothesis::discriminator: discriminator with label '" + l + "' not set");
      }
      return it->second;
  }
